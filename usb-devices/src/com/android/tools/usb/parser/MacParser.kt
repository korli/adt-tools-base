...
class MacParser : OutputParser {
    ...
    object USBDeclarationCollector : Collector<String, MutableList<MutableList<String>>, MutableList<MutableList<String>>> {
        ...
        override fun finisher(): Function<MutableList<MutableList<String>>, MutableList<MutableList<String>>> = Function.identity<MutableList<MutableList<String>>>()
    }

    override fun parse(output: InputStream): CompletableFuture<List<UsbDevice>> {
        val lines = BufferedReader(InputStreamReader(output))
                .lines().skip(1) // skip the first line USB: output
        return CompletableFuture.supplyAsync({
            lines.collect(USBDeclarationCollector)
